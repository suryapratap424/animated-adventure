...
export default class Vector {
  ...
  draw(ctx, [a, b], l = 1) {
    ...
    if (this.mode == "arrow") {
      drawArrow(ctx, a, b, x, y, this.weight, this.color);
    } else {
      ctx.beginPath();
    ...
    if (this.MagOptions.is) {
      if (this.MagOptions.direction == "TIP")
        ctx.fillText(this.mag().toFixed(2), x + 2, y + 2, 50);
      if (this.MagOptions.direction == "MID")
        ctx.fillText(this.mag().toFixed(2), (a+x) / 2, (b+y) / 2, 50);
      if (this.MagOptions.direction == "TAIL")
        ctx.fillText(this.mag().toFixed(2), a, b, 50);
    }
  }
